...
/* This is a simple STB style hashtable library using the following paper for
   reference regarding using a bitmap for pointers into the buckets:
...
/* dt_reset(dt) resets dt to an empty state, setting its active capacity back
   to INITIAL_CAPACITY. All stored items are removed without unmapping memory.
*/
...
/* hash_key(key) is a helper hash function.
   (It is public so you can use it externally if desired.)
*/
...
#define LOAD_FACTOR_UPPER 0.75    /* (Not used directly here, growth triggered on bucket fullness) */
#endif
...
/* dt_reset() clears all stored items and resets the active capacity
   back to INITIAL_CAPACITY without unmapping any memory.
*/
void dt_reset(dtable_t *dt) {
    if (!dt) return;
    dt->count = 0;
    dt->current_capacity = INITIAL_CAPACITY;
    /* Clear keys and values for the active region */
    memset(dt->keys, 0, dt->current_capacity * sizeof(uint32_t));
    memset(dt->values, 0, dt->current_capacity * sizeof(uint32_t));
    /* Clear bitmap for active buckets (active buckets = current_capacity/BUCKET_SIZE) */
